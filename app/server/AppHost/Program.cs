...
// Set environment variables FIRST before Aspire initialization
// This bypasses DCP/Dashboard requirements for .NET 10 preview
Environment.SetEnvironmentVariable("ASPIRE_SKIP_DASHBOARD", "true");
Environment.SetEnvironmentVariable("ASPIRE_SKIP_PREREQUISITES", "true");
Environment.SetEnvironmentVariable("ASPIRE_ALLOW_UNSECURED_TRANSPORT", "false");
Environment.SetEnvironmentVariable("DOTNET_ASPIRE_DISABLE_DASHBOARD", "true");
...
catch (Exception ex) when (ex.Message.Contains("CliPath") || ex.Message.Contains("DashboardPath") || ex.Message.Contains("DCP"))
{
    // DCP/Dashboard not available in .NET 10 preview
    Console.WriteLine("ERROR: Aspire DCP/Dashboard issue detected");
    Console.WriteLine("Use run-services.ps1 instead:");
    Console.WriteLine("  .\\app\\server\\run-services.ps1");
    Console.WriteLine("");
    Console.WriteLine("Or install Aspire.Dashboard.Standalone:");
    Console.WriteLine("  dotnet tool install Aspire.Dashboard.Standalone --global");
    Console.WriteLine("  aspire-dashboard");
    Environment.Exit(1);
}
catch (Exception ex)
{
