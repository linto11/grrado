...
public class DataSeedingService : IDataSeedingService
{
    ...
    {
        // Try to find the data directory by checking multiple possible locations
        _dataPath = FindDataPath(dataPath);
        Console.WriteLine($"Using data path: {_dataPath}");
    }

    private string FindDataPath(string preferredPath)
    {
        ...
        var possiblePaths = new[]
        {
            ...
            "/src/data", // Linux path
            "D:\\_GRRADO\\src\\data" // Full path
        };

        foreach (var path in possiblePaths)
        {
            var fullPath = Path.GetFullPath(path);
            Console.WriteLine($"Checking data path: {fullPath} - exists: {Directory.Exists(fullPath)}");
        ...
        // If nothing found, return the preferred path and let it fail gracefully
        Console.WriteLine($"Warning: No data directory found, using: {Path.GetFullPath(preferredPath)}");
    ...
    private async Task SeedUsersAsync(VehicleServiceDbContext context)
    {
        ...
        Console.WriteLine($"Looking for users file at: {filePath}");
        Console.WriteLine($"File exists: {File.Exists(filePath)}");
        if (!File.Exists(filePath)) 
        {
            Console.WriteLine($"Users.csv not found at {filePath}");
            return;
        }
